...
$pageTitle = ($isEdit ? "แก้ไข" : "เพิ่ม") . "รายการไปราชการ — BICC OVEC";
?>
<!DOCTYPE html>
<html lang="th">
<head>
...
<aside class="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <a class="nav-a active" href="/bicc-ovec/user/travel_form.php"><i class="fas fa-plus-circle"></i> เพิ่มการไปราชการ</a>
...
<main class="main">
  <div class="page-header">
    ...
    <div>
      <div class="page-title" id="formTitle">📝 เพิ่มรายการไปราชการใหม่</div>
      <div class="page-sub">กรอกข้อมูลให้ครบถ้วน แล้วกดบันทึก (กระบวนการที่ 1)</div>
    </div>
  </div>
  ...
  <form id="travelForm">
    ...
    <div class="btn-row">
      <a href="/bicc-ovec/user/dashboard.php" class="btn btn-ghost"><i class="fas fa-times"></i> ยกเลิก</a>
...
onAuthStateChanged(auth, async user => {
  ...
  if(editId){
    document.getElementById('formTitle').textContent = '✏️ แก้ไขรายการไปราชการ';
    await loadEdit(editId, ud);
  } else {
    // Auto-fill user info
    document.getElementById('travelerName').value = ud.displayName||'';
    document.getElementById('position').value = ud.position||'';
    document.getElementById('department').value = ud.department||'';
  }
...
document.getElementById('travelForm').addEventListener('submit', async e => {
  ...
  try {
    if(editId){
      ...
      okBox.classList.add('show'); document.getElementById('okText').textContent='บันทึกข้อมูลสำเร็จ! รอการอนุมัติจากผู้ดูแลระบบ';
    }
    btn.classList.remove('loading'); btn.disabled=false;
    setTimeout(()=>window.location.href='/bicc-ovec/user/dashboard.php', 1800);
  } catch(err){
    btn.classList.remove('loading'); btn.disabled=false;
